...
Experiment3 checks how the precession velocity depends on different parameters (period time, orbit size, expansion rate, eccentricity). It makes a big table where you can see the formula values and the deviation between the simulation and the literature. It runs on many loops to try all combinations of parameters.
(I saved an example result in the file expr3_results.xlsx.)
...
--------------------------------------
*_Certesian functions are functions written in the Descartes coordinate system.
*_Polar functions are functions written in the polar coordinate system.

DiffBase_* are the differential equations of the basic two-body problem.
The diffExpansionNaive_* is the naive expansion model, where expansion only increases distances.
DiffExpansion_* is the exact expansion model, which takes into account the decrease in the peculiar velocity.
